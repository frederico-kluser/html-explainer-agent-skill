...
} from '../html-explainer-agent-skill/scripts/new-builder.mjs';
...
describe('gerador — byte a byte: o bloco pré-preenchido é o que o runtime monta', () => {
  const t = sandbox('html-explainer-agent-skill-gerador-');
  after(() => t.rm());
...
//
// O install.sh instala a skill como symlink do DIRETÓRIO html-explainer-agent-skill/ inteiro. O loader
...
describe('invocado por um symlink (é assim que o install.sh instala a skill)', () => {
  const t = sandbox('html-explainer-agent-skill-symlink-');
  after(() => t.rm());

  before(() => symlinkSync(resolve(ROOT, 'html-explainer-agent-skill'), t.path('skill-link'), 'dir'));
